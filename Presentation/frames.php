...
<header>
  ...
  <div class="bg-primary mb-4" id="backgroundDiv">
    <div class="container py-4">
      <h3 class="text-white mt-2">Frames</h3>
      <!-- Breadcrumb -->
      <nav class="d-flex mb-2">
        <h6 class="mb-0">
          <a href="../Presentation/home.php" class="text-white-50">Home</a>
          <span class="text-white-50 mx-2"> > </span>
          <a href="../Presentation/frames.php" class="text-white"><u>Frames</u></a>
        </h6>
      </nav>
      <!-- Breadcrumb -->
    </div>
...
<section class="">
  <div class="container">
    ...
    $sql = "SELECT * FROM product WHERE category_name = 1 LIMIT $items_per_page OFFSET $offset";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '</div>';
            echo '</div>';
            echo '</a>'; // Closed the <a> tag for product details
            echo '</div>';
        }

        // Pagination controls
        $result_total = $conn->query("SELECT COUNT(*) as total FROM product WHERE category_name = 1");
